...
function App() {
  ...
  async function updateComputer(partial: Partial<Computer>) {
    ...
    try {
      ...
      await axios.post(`/api/computer/${partType}`, { id: Object.values(partial)[0].id });
    } catch (error) {
      console.log(error)
      setComputer(before)
      setLoggedIn(false)
  ...
  async function getPart(endpoint: string, id: string): Promise<BasePart | undefined> {
    try {
  ...
  return (
    <div className="flex flex-row h-full">
      <div className="flex flex-col flex-1 items-center justify-center gap-4 p-8 items-center">
        ...
        {!loggedIn ? (
          <>
            <p className="text-center">You are currently logged out, your changes will not be saved.<br />Create or login to your account to save your computer.</p>
        ...
        <div className="flex flex-col w-full h-full bg-stone-700 rounded-3xl p-4 gap-4 overflow-auto">
          ...
          {!computerLoading ? (Object.entries(computer).map(([partType, part]) => (
            part ?
              (<div className="group flex flex-row justify-between items-center">
                ...
                <div className="flex flex-row justify-between items-center text-stone-400">
                  ...
                  <button
                    onClick={() => getData(partType.toLowerCase().replaceAll('_', '-'))}
      ...
      <div className="flex flex-col flex-1 items-center gap-4 p-8">
        ...
        <div className="flex flex-col w-full overflow-auto flex-grow">
          ...
          {!dataLoading ? (data && data.map((item) => (
            <div className="flex flex-row items-center justify-between py-1 px-4">
              ...
              <div className="flex flex-row gap-2 items-center">
                ...
                <button
                  onClick={() => updateComputer({ [selectedPartType.toLowerCase().replaceAll(' ', '_')]: item })}
                  className="font-bold"
                >
                  +
                </button>
